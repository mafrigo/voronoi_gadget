...
def voronoimap(snap, qty='vel', grid=None, extent=20, npixel_per_side=200, partperbin=None, nspaxels=500,
               weightqty='mass', sigmapp=-1., npseudoparticles=60, force_orient=True, ensure_rotdir=True,
               artificial_error=0., measerror=0., scalebar=None, cutatmag=None, figureconfig='horizontal', info=None,
               plotfile=None, savetxt=None, savefigure=True, custom_titles=None):
    """
    ...
    ensure_rotdir   : If True, the rotation direction will be forced to be always the same by flipping the snapshot if
                      necessary.
    artificial_error: If specified, quantity is randomly displaced according to a gaussian with sigma=artificial_error.
    measerror       : The measurement error of every value; only affects the calculation if statsmode='likelihood'.
